...
func onInitialize() {
	if configFile != "" {
		...
		for _, dir := range searchDirs() {
			fmt.Println("Searching for config file in:", dir)
			// 将 dir 目录加入到配置文件的搜索路径，AddConfigPath 添加默认配置文件
...
func setupEnvironmentVariables() {
	// 允许 viper 自动匹配环境变量
	viper.AutomaticEnv()
	// 设置环境变量前缀，当多个服务同时在一台机器上运行时，可能会出现环境变量名称冲突，所以加一个前缀。
	viper.SetEnvPrefix("MINIBLOG")
	// 替换环境变量 key 中的分隔符 '.' 和 '-' 为 '_'，确保环境变量命名与配置键名兼容
